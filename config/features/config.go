...
type Flags struct {
	...
	EnableHezeTicketCache               bool // EnableHezeTicketCache routes Gloas FCU to engine_forkchoiceUpdatedV5 and caches the active blob-streaming tickets it returns.
...
func ConfigureBeaconChain(ctx *cli.Context) error {
	...
	if ctx.IsSet(enableHezeTicketCache.Name) {
		logEnabled(enableHezeTicketCache)
		cfg.EnableHezeTicketCache = true
	}
