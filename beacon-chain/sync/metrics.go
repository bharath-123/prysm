...
var (
	...
	attestationArrivalGossipSummary = promauto.NewSummary(
		prometheus.SummaryOpts{
			Name: "gossip_attestation_arrival_milliseconds",
			Help: "Time for gossiped attestations to arrive",
		},
	)
	...
	signatureBatchSizes = promauto.NewHistogram(
		prometheus.HistogramOpts{
			Name:    tempMetricPrefix + "signature_batch_sizes",
			Help:    "Captures the size of signature batches.",
			Buckets: []float64{1, 5, 10, 20, 50, 100, 200, 500, 1000, 2000, 5000, 10000},
		},
	)

	batchVerifierProcessingTime = promauto.NewHistogram(
		prometheus.HistogramOpts{
			Name:    tempMetricPrefix + "batch_verifier_processing_time_microseconds",
			Help:    "Captures the time taken to process a batch verifier.",
			Buckets: []float64{100, 500, 1_000, 5_000, 10_000, 50_000, 100_000, 250_000, 500_000, 1_000_000, 5_000_000, 10_000_000, 20_000_000, 30_000_000},
		},
	)

	batchVerifierMaxSize = promauto.NewGauge(prometheus.GaugeOpts{
