...
type EngineCaller interface {
	NewPayload(ctx context.Context, payload interfaces.ExecutionData, versionedHashes []common.Hash, aotVersionedHashes [][]common.Hash, parentBlockRoot *common.Hash, executionRequests *pb.ExecutionRequests) ([]byte, error)
	ForkchoiceUpdated(
...
func (s *Service) NewPayload(ctx context.Context, payload interfaces.ExecutionData, versionedHashes []common.Hash, aotVersionedHashes [][]common.Hash, parentBlockRoot *common.Hash, executionRequests *pb.ExecutionRequests) ([]byte, error) {
	...
	case *pb.ExecutionPayloadGloas:
		...
		// Blob Streaming: pass the JIT and AOT (per-ticket, list-of-lists) blob versioned
		// hashes separately so the EL can validate each class against the payload's txs.
		err = s.rpcClient.CallContext(ctx, result, NewPayloadMethodV5, payloadPb, versionedHashes, aotVersionedHashes, parentBlockRoot, flattenedRequests)
		if err != nil {
			return nil, handleRPCError(err)
		}
	default:
