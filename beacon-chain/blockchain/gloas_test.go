...
func TestNotifyForkchoiceUpdateGloas_Valid(t *testing.T) {
	...
	retPid, err := s.notifyForkchoiceUpdateGloas(ctx, nil, [32]byte{}, blockHash, 0, attr, false, "test")
	require.NoError(t, err)
...
func TestNotifyForkchoiceUpdateGloas_Syncing(t *testing.T) {
	...
	blockHash := bytesutil.ToBytes32([]byte("hash1"))
	_, err := s.notifyForkchoiceUpdateGloas(ctx, nil, [32]byte{}, blockHash, 0, nil, false, "test")
	require.NoError(t, err)
}
